...
def test_improved_model():
    ...
    # 🆕 保存预测数据与真实数据到my_data.csv
    my_data = []
    for item in predictions_data:
        my_data.append({
            'filename': item['filename'],
            'predicted_value': item['predicted'],
            'true_value': item['actual'],
            'is_correct': item['correct']
        })
    
    my_data_file = 'my_data.csv'
    pd.DataFrame(my_data).to_csv(my_data_file, index=False)
    print(f"📊 预测vs真实数据已保存到: {my_data_file}")
