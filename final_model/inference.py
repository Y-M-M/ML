...
def test_improved_model():
    ...
    # 额外保存到my_data.csv，包含详细对比信息
    my_data_output = []
    for data in predictions_data:
        my_data_output.append({
            '文件名': data['filename'],
            '预测数据': data['predicted'],
            '实际数据': data['actual'],
            '对比结果': '正确' if data['correct'] else '错误',
            '原始预测': data['raw_prediction'],
            '边界框来源': data['bbox_source'],
            '是否匹配': '✅' if data['correct'] else '❌'
        })
    
    my_data_df = pd.DataFrame(my_data_output)
    my_data_df.to_csv('my_data.csv', index=False, encoding='utf-8-sig')
    print(f"💾 详细对比结果已保存到: my_data.csv")
